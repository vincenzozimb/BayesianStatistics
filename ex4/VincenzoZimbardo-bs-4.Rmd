...
sample_alpha <- function(z, c){
    # input control
    stopifnot(nrow(z)==n && ncol(z)==k)
    stopifnot(length(c)==k) # hyper-parameter for the Dirichlet distribution f(alpha)
    # function
    z_plus <- colSums(z)
    c_new <- c + z_plus
    # return
...
sample_p <- function(z, a, b){
    # input control
    stopifnot(nrow(z)==n && ncol(z)==k)
    stopifnot(length(a)==k && length(b)==k) # hyper-parameters for the beta distributions f(p)
    # function
    y_plus <- y %*% z
    m_plus <- m %*% z
    a_new <- a + y_plus
...
sample_z <- function(alpha, p){
    ...
    for (i in i:n) {
       f[i] <- sample(c(1:k), size=1, prob=r[i,])
    }
...
# Gibbs sampler
gibbs <- function(n, alpha_0, p_0, z_0, a, b, c){

    # input control
    stopifnot(nrow(z_0)==n && ncol(z_0)==k)
    stopifnot(length(alpha_0)==k)
    stopifnot(length(p_0)==k)
    stopifnot(length(a)==k && length(b)==k && length(c)==k)
    ...
    alpha_samples <- matrix(nrow = n, ncol = k)
    p_samples <- matrix(nrow = n, ncol = k)

    # iteration
    for (i in 1:n) {
        alpha_curr <- sample_alpha(z_curr, c)
        p_curr <- sample_p(z_curr, a, b)
...
binomial_mixture <- "model {
    ...
    # Likelihood
    for (i in 1:n) { 
        y[i] ~ dbin(gam[q[i]], m[i])
    }

}"

# Specify desired parameters for inference
parameters <- c("alpha", "gam")
...
MCMCrun <- jags(
  ...
  n.burnin = n.burnin,
  n.iter = n.thin*nsamples + n.burnin,
  n.chains = 1,
  n.thin = n.thin,
  progress.bar = "none",
  quiet = TRUE,
...
samples <- cbind(samples_list$alpha, samples_list$gam)
names <- c()
for (i in 1:num_params) {
   names <- c(names, paste0("alpha[", i, "]"))
}
for (i in 1:num_params) {
   names <- c(names, paste0("gamma[", i, "]"))
}
...
par(mfrow = c(1, 2))
plot(1:nsamples,samples[,1], type="l", main=NULL, xlab = expression(i), ylab = expression(alpha[0]^(i)))
plot(1:nsamples,samples[,num_params+1], type="l", main=NULL, xlab = expression(i), ylab = expression(gamma[0]^(i)))
...
par(mfrow = c(1, 2))
for (i in 1:num_params) {
    hist(samples[,i], main=colnames(samples)[i], xlab=NULL)
    hist(samples[,num_params+i], main=colnames(samples)[num_params+i], xlab=NULL)
}
# Restore default plot options
par(mfrow = c(1, 1))
```
