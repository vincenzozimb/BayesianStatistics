...
repeat{
    ...
    accuracy <- c(accuracy, prob[count] * (1 - prob[count]) / length(x_ar))
    if(accuracy[count] < treshold && count > min){
        break
    }
}
...
par(mfrow = c(1, 1))
```
Estimate the probability that the coin is biased toward head, that is $\mathbb{E}(\mathbb{1}_{\theta > 0})$.
It is simply the number of positive sample points over the total size of the sample 
The estimated probability is $\mathbb{P}(\theta>0)=$ `r prob[length(prob)]`, calculated with a sample of $n=$ `r length(x_ar)`
```{r echo=FALSE}
plot(seq(1,count,1),accuracy, pch="*", cex=0.5)
```
