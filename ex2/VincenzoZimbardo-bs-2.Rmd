...
```{r echo=TRUE, warning=FALSE}
start1        <- c(1500, 3, 3)
optimization1 <- optim(par=start1, fn=logPost, control=list(fnscale=-scaling), method = "BFGS", hessian = TRUE)
...
```{r echo=TRUE, warning=FALSE}
start2        <- c(1500, 1, 1)
optimization2 <- optim(par=start2, fn=logPost, control=list(fnscale=-scaling), method = "BFGS", hessian = TRUE)
...
```{r echo=TRUE, warning=FALSE}
start3        <- c(1500, 10, 10)
optimization3 <- optim(par=start3, fn=logPost, control=list(fnscale=-scaling), method = "BFGS", hessian = TRUE)
...
## Find a 90% interval estimates for the log of the standard deviations $\log\sigma_y$, $\log\sigma_b$

```{r echo=TRUE}
mode <- mode1
H <- optimization1$hessian
V <- -solve(H)
