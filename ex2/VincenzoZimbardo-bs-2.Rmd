...
Let $N=6$ be the number of batches and $n=5$ the number of observations in each batch.
First of all, write down a function to calculate the within sum of squares (SSW) for each group:
\begin{equation}
    SSW_i = \sum_{j=1}^{n} (y_{ij} - \bar{y}_i)^2
\end{equation}
