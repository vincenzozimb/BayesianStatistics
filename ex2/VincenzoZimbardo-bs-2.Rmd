...
The estimated mean value and standard deviation from the Poisson approaximation are
```{r echo=FALSE}
...
Z_prop <- gamma(dof/2) * sqrt(pi*dof*var) / gamma((dof+1)/2) 
# Proposal kernel
ProposalKernel <- function(x){
    return( Proposal(x) * Z_prop )
}
...
```{r echo=TRUE}
# Sample from from the posterior using the acc-rej algorithm.
optim <- optim(par=0.0, fn= function (x) PosteriorKernel(x) / Proposal(x), method = "BFGS",  control=list(fnscale=-1))
K <- optim$value
K <- K / Z_prop
...
y <- PosteriorKernel(x)
par(mfrow = c(1, 3))
plot(x,y, type="l", ylab="Posterior kernel", yaxt="n")
y <- Proposal(x)
plot(x,y, type="l", ylab="Proposal t")
hist(x_ar, main="")
# Restore default plot options
...
```{r echo=FALSE}
print(paste0("Mean value: ", round(mean(x_ar),4)))
print(paste0("Standard deviation: ", round(varest[count],4)))
```
