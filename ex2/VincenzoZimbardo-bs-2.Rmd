...
\begin{equation}
    \log(g(\theta \vert data)) = 2N(\theta_2+\theta_3) + \sum_{i=1}^{N} \phi(\bar{y_i} \vert \theta_1, \sigma) + \sum_{i=1}^{N} f_G(S_i \vert a, b)
\end{equation}


```{r echo=TRUE}
# define auxiliary parameter and functions
a <- (n-1) / 2
sigma <- function(theta2, theta3){
    return ( sqrt( exp(2*theta2)/n + exp(2*theta3) ) )
}
b <- function(theta2){
    return ( exp(-2*theta2) / 2 )
}
# Normal density
phi <- function(y, theta){
    t1 <- theta[1]
    t2 <- theta[2]
    t3 <- theta[3]
    return ( dnorm(y, t1, sigma(t2, t3)) )
}
# Gamma density
fG <- function(S, theta){
    t1 <- theta[1]
