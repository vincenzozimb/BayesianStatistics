...
M <- optim$value
print(paste0(c("M=",M), collapse=""))
```

```{r echo=FALSE}
# plot
x <- seq(-2, 2, length.out = 100)
y <- M * dnorm(x,mode,sqrt(var))
...
repeat{
    ...
    if(abs(varest[count]-varest[count-1]) < treshold && count > min){
        break
    }
}
...
stabilizes around a limiting value, up to variations below a predetermined treshold.
...
Moreover:
\begin{equation}
    \sum_{i=1}^{N} e^{\beta_1 i} = \frac{1-e^{(N+1)\beta_1}}{1-e^{\beta_1}} =: f(\beta_1)
\end{equation}
...
# Auxiliary function
func <- function(x){
    res <- ( 1-exp((N+1)*x) ) / ( 1-exp(x) )
    return (res)
}
# log posterior, up to the additive constant (related to the normalization)
logPost <- function(beta0, beta1){
    res <- A*beta0 + D*beta_1 - func(beta_1)*exp(beta_0)
    return (res)
}
# Posterior kernel
PosteriorKernel <- function(beta0, beta1){
    return (exp(logPost(beta0, beta1)))
}
```
Marginalize to find the posterior for the slope $\beta_1$
```{r echo=TRUE}
# Marginalize
SlopePosteriorKernel <- function(beta1){
  result <- integrate(function(beta0) Posterior(beta0, beta1), lower = -Inf, upper = Inf)
  return(result$value / integralConstant)
}
# Normalize
# Z_post <- integrate(function(x) (SlopePosteriorKernel(x)), -Inf, Inf)$value
# SlopePosterior <- function(beta1){
#     return ( SlopePosteriorKernel(beta1) / Z_post )
# }
```
```{r echo=FALSE}
# plot
xx <- seq(-0.5, 0.5, length.out = 1000)
yy <- sapply(xx, SlopePosteriorKernel)
plot(xx, yy, type="l", main="Slope posterior", xlab=expression(beta[1]), ylab=expression(g[beta[1]]))
```
