...
prior_sigma2 <- function(sigma2){
  return (dinvgamma(sigma2, shape=a, rate=b))
}
# Zellner's g-prior on beta | sigma^2
prior_beta <- function(beta, sigma2){
  stopifnot(length(beta) == 3)
  S <- g*sigma2 * Y
  return ( dmvnorm(beta, B, S) )
}
# Likelihood
...
posterior <- function(beta, sigma2){
  stopifnot(length(beta) == 3)
  return ( likelihood(beta, sigma2) * prior_beta(beta, sigma2) * prior_sigma2(sigma2) )
}
...
The posteriorn density is, up to the normalizing constant (I omit the data in the notation for simplicity)

\begin{equation}
  f(\beta, \sigma^2) \propto (\sigma^2)^{-N/2-3/2-a-1} \exp\left[-\frac{1}{2\sigma^2}\sum_{i=1}^{N}(y_i-x_i^T\beta)^2 \right] \exp\left[ \frac{-1}{2\sigma^2g}(\beta-B)^T(X^TX)(\beta-B) \right] e^{-b/\sigma^2}
\end{equation}

And the full conditionals are:

\begin{equation}
  f(\sigma^2 | \beta) \propto (\sigma^2)^{-N/2-3/2-a-1} \exp\left[-\frac{1}{\sigma^2}\left(\frac{1}{2}\sum_{i=1}^{N}(y_i-x_i^T\beta)^2 + \frac{1}{2g}(\beta-B)^T(X^TX)(\beta-B) +b \right) \right]
\end{equation}

for $\sigma^2 | \beta$, which is an inverse-gamma distribution, let's say $\sigma^2 | \beta \sim \mathcal{IG}(\tilde{a}, \tilde{b})$, and

\begin{align}
  f(\beta | \sigma^2) &\propto \exp\left[-\frac{1}{2\sigma^2}\left(\sum_{i=1}^{N}(y_i-x_i^T\beta)^2 + \frac{1}{g}(\beta-B)^T(X^TX)(\beta-B)\right) \right] \\
    &= \exp\left[ -\frac{1}{2\sigma^2} \left( (X^{-1}y-\beta)^TX^TX(X^{-1}y-\beta) + \frac{1}{g}(B-\beta)X^TX(B-\beta) \right) \right]
\end{align}

for $\beta | \sigma^2$, which is the product of two normal densities, with mean values $X^{-1}y$ and $B$ respectively, 
and covariance matrices that differ only for a factor $1/g$. 

In general, for two multivariate normal densities of parameters $(\mu_1, \Sigma_1)$ and $(\mu_2, \Sigma_2)$, the product is again a normal density
of parameters $(\mu, \Sigma)$ given by
...
\begin{align}
  \mu &=  \\
  \Sigma &= \frac{1}{2\sigma^2(1+g)}X^TX
\end{align}

Given the form of the two full conditionals, it might be worth to implement a Gibbs sampler.

```{r echo=TRUE}
# Function to sample from the conditional distribution of sigma^2 given beta
sample_s2_given_b <- function(sigma2, beta){
  stopifnot(length(beta) == 3)
  # parameter
  aa <- a + N/2 + 3/2
  bb <- (sum((y - X%*%beta)^2) + t(beta-B)%*%XX%*%(beta-B) / g + b) / 2 
  # proposal density
  res <- rinvgamma(sigma2, shape=aa, rate=bb)
  return (res)
}
# Function to sample from the conditional distribution of beta given sigma^2
sample_b_given_s2 <- function(beta, sigma2){
  stopifnot(length(beta) == 3)
  # parameter
  m <- solve(X)%*%y
  mu <- B
