...
The net effect of the correlation is to reduce the effective sample size.
...
Finally, estimates for $\log(\lambda_A)$ and $\log(\lambda_B)$ can be calculated:
...
Also in this case no burn-in phase is necessary.
Moreover the sample is practically decorrelated, and we can regard the sample as an IID sample.
...
hist(sigma_simul, main=NULL, xlab = expression(sigma^2))
par(mfrow = c(1, 1))
```
...
CI_inf <- qnorm(0.05, mean=CI_mean, sd=sigma_hat)
CI_sup <- qnorm(0.95, mean=CI_mean, sd=sigma_hat)
...
Consistent with the fact of having used a sample with larger variance (the `blinreg` sample). 
...
### Run the chain using the following initial values: $\alpha_0=4$, $\beta_0=0$, $\gamma=1$, $\tau_\alpha=1$, $\tau_\beta=1$ and $\tau=1$. Run a burn in of $1000$ updates followed by a futher $10000$ updates.
...
For the choice of the prior parameters the book @gilks1995markov was used as a reference.
...
```{r echo=TRUE}
CI_lower <- round(y_mean+qnorm(0.025)*y_sd/sqrt(num_samples),4)
CI_upper <- round(y_mean+qnorm(0.975)*y_sd/sqrt(num_samples),4)
CI <- cbind(CI_lower, CI_upper)
names <- c()
for (i in 1:nrow(CI)) {
...
y0 <- mean(colMeans(samples_list$y0))
