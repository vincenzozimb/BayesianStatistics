...
\begin{align}
  ...
  &= \prod_{i=1}^{N} \binom{n_i}{y_i} e^{g_i(\beta)y_i} \left( \frac{e^{g_i(\beta)}}{1+e^{g_i(\beta)}} \right)^{n_i}
\end{align}

Therefore the log-posterior density is given, up to an additive constant, by 

\begin{equation}
  \log{f(\beta | \textbf{y})} = \sum_{i=1}^{N} \left[ \log{\binom{n_i}{y_i}} + g_i(\beta) (y_i + n_i) - n_i \log{(1+e^{g_i(\beta)})} \right]
\end{equation}
...
logposterior <- function(beta){
  stopifnot(length(beta) == 3)
  g <- g_func(beta)
  res <- lchoose(n,y) + g*(y+n) - n*log(1+exp(g))
  res <- sum(res)
  return (res)
}
```
...
proposal <- function(beta){
  return (dmvnorm(beta, mode, V, log=FALSE))
}
```
